			    ...
			    key_serial_t ringid)
{
	...
	ret = -EPERM;
	instkey = task_aux(current)->request_key_auth;
	if (!instkey)
		goto error;
	...
	if (ret == 0) {
		key_put(task_aux(current)->request_key_auth);
		task_aux(current)->request_key_auth = NULL;
	}
...
long keyctl_set_reqkey_keyring(int reqkey_defl)
{
	...
	set:
		task_aux(current)->jit_keyring = reqkey_defl;

	case KEY_REQKEY_DEFL_NO_CHANGE:
		return task_aux(current)->jit_keyring;
...
long keyctl_assume_authority(key_serial_t id)
{
	...
	key_put(task_aux(current)->request_key_auth);
	task_aux(current)->request_key_auth = authkey;
	ret = authkey->serial;
