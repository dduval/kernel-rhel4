...
int install_thread_keyring(struct task_struct *tsk)
{
	...
	task_lock(tsk);
	old = task_aux(tsk)->thread_keyring;
	task_aux(tsk)->thread_keyring = keyring;
	task_unlock(tsk);
...
void key_fsuid_changed(struct task_struct *tsk)
{
	...
	if (task_aux(tsk)->thread_keyring) {
		down_write(&task_aux(tsk)->thread_keyring->sem);
		task_aux(tsk)->thread_keyring->uid = tsk->fsuid;
		up_write(&task_aux(tsk)->thread_keyring->sem);
	}
...
void key_fsgid_changed(struct task_struct *tsk)
{
	...
	if (task_aux(tsk)->thread_keyring) {
		down_write(&task_aux(tsk)->thread_keyring->sem);
		task_aux(tsk)->thread_keyring->gid = tsk->fsgid;
		up_write(&task_aux(tsk)->thread_keyring->sem);
	}
