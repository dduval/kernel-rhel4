...
static int snd_nm256_pcm_hw_params(snd_pcm_substream_t *substream,
 ...
 * event.
 */
static int nm256_suspend(struct pci_dev *pci, unsigned int state)
{
	...
	pci_save_state(pci, chip->pci_cfg_state);
	return 0;
}
...
static int nm256_resume(struct pci_dev *pci)
{
	...
	pci_restore_state(pci, chip->pci_cfg_state);
...
static int snd_nm256_free(struct nm256 *chip)
{
	...
	if (chip->cport) {
		iounmap(chip->cport);
		release_resource(chip->cport);
		kfree_nocheck(chip->cport);
	}
	if (chip->buffer) {
		iounmap(chip->buffer);
		release_resource(chip->buffer);
		kfree_nocheck(chip->buffer);
	}
	if (chip->irq >= 0)
		free_irq(chip->irq, chip);
