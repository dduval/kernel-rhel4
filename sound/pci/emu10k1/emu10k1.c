					    ...
					    const struct pci_device_id *pci_id)
{
	...
	if (emu->audigy && ((emu->serial == 0x10221102) || (emu->serial == 0x10211102))) {
		strcpy(card->driver, "Audigy4");
		strcpy(card->shortname, "Sound Blaster Audigy4");
	} else if (emu->audigy && (emu->serial == 0x10011102) ) {
