...
static int stac92xx_dmux_enum_info(snd_kcontrol_t *kcontrol,
...
static int stac92xx_dmux_enum_get(snd_kcontrol_t *kcontrol,
...
static int stac92xx_dmux_enum_put(snd_kcontrol_t *kcontrol,
				  snd_ctl_elem_value_t *ucontrol)
{
	struct hda_codec *codec = snd_kcontrol_chip(kcontrol);
	struct sigmatel_spec *spec = codec->spec;
...
static snd_kcontrol_new_t stac925x_mixer[] = {
...
static snd_kcontrol_new_t vaio_ar_mixer[] = {
	{
		.iface = SNDRV_CTL_ELEM_IFACE_MIXER,
		.name = "Master Playback Volume",
		.info = snd_hda_mixer_amp_volume_info,
