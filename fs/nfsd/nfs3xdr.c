...
encode_fattr3(struct svc_rqst *rqstp, u32 *p, struct svc_fh *fhp)
{
	...
	*p++ = htonl((u32) MINOR(stat.rdev));
	...
	p = xdr_encode_hyper(p, stat.ino64);
...
encode_saved_post_attr(struct svc_rqst *rqstp, u32 *p, struct svc_fh *fhp)
{
	...
	*p++ = htonl((u32) MAJOR(fhp->fh_post_attr.rdev));
	*p++ = htonl((u32) MINOR(fhp->fh_post_attr.rdev));
	if (is_fsid(fhp, rqstp->rq_reffh))
		p = xdr_encode_hyper(p, (u64) fhp->fh_export->ex_fsid);
	else
...
void fill_post_wcc(struct svc_fh *fhp)
{
	...
	err = vfs_getattr64(fhp->fh_export->ex_mnt, fhp->fh_dentry,
			&fhp->fh_post_attr);
	if (err)
		fhp->fh_post_saved = 0;
	else
					...
					struct nfsd3_writeargs *args)
{
	...
	dlen = rqstp->rq_arg.head[0].iov_len + rqstp->rq_arg.page_len - hdr;
	/*
...
int
nfs3svc_encode_entry32(struct readdir_cd *cd, const char *name,
		     int namlen, loff_t offset, u32 ino, unsigned int d_type)
{
	return encode_entry(cd, name, namlen, offset, (u64) ino, d_type, 0);
}
...
int
nfs3svc_encode_entry_plus32(struct readdir_cd *cd, const char *name,
			  int namlen, loff_t offset, u32 ino,
			  unsigned int d_type)
{
	return encode_entry(cd, name, namlen, offset, (u64) ino, d_type, 1);
}
