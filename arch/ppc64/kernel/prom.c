 ...
 */
static int fixup_added_cpu(struct device_node *np)
{
	unsigned int cpu;
...
/*
 * Update the present map for a cpu node which is going away.
 */
static void fixup_removed_cpu(struct device_node *np)
{
	unsigned int cpu;
	int len, nthreads, i;
	u32 *intserv;
...
static int of_finish_dynamic_node(struct device_node *node)
{
	...
	if (node->type && !strcmp("cpu", node->type))
		err = fixup_added_cpu(node);
	if (err)
		goto out;
...
int of_remove_node(struct device_node *np)
{
	...
	if (np->type && !strcmp("cpu", np->type))
		fixup_removed_cpu(np);
