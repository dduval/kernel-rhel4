...
extern unsigned long pci_mem_start;  /* See arch/x86_64/kernel/setup-xen.c */
...
unsigned long __init e820_end_of_ram(void)
{
	...
	if (xen_override_max_pfn == 0) {
		...
		if ( rc == -ENOSYS ) {
			...
			max_end_pfn = map[0].size >> PAGE_SHIFT;
		}
	...
	/*
	 * Fedora doesn't have this, but or initial RHEL-4 code uses
	 * end_pfn_map. So, stealing the sad side effect section from
	 * the old non-Xen instance of e820_end_of_ram.
	 */
	if (max_end_pfn > end_pfn_map) 
		end_pfn_map = max_end_pfn;
	if (end_pfn_map > MAXMEM>>PAGE_SHIFT)
		end_pfn_map = MAXMEM>>PAGE_SHIFT;
...
void __init e820_reserve_resources(void) 
{
#if 0 /* see other if-0 comment below */
	dom0_op_t op;
