...
static void mm_walk(struct mm_struct *mm, pgprot_t flags)
{
	 ...
	 * (In RHEL-4, TASK_SIZE corresponds to TASK_SIZE64 of Fedora)
	 */
	for (g = 0; g <= ((TASK_SIZE-1) / PGDIR_SIZE); g++, pgd++) {
		if (pgd_none(*pgd))
			continue;
...
void mm_pin(struct mm_struct *mm)
{
	...
	mm->context.pinned = 1;
	make_page_readonly(pml4, XENFEAT_writable_page_tables);
...
void mm_unpin(struct mm_struct *mm)
{
	...
	/* The PML4 is always pinned, see pgd_alloc. */
	set_pml4(pml4, __pml4(0));
	xen_pgd_unpin(__pa(pml4));
	BUG_ON(HYPERVISOR_update_va_mapping(
		       (unsigned long)pml4,
		       pfn_pte(virt_to_phys(pml4)>>PAGE_SHIFT, PAGE_KERNEL),
		       0));
