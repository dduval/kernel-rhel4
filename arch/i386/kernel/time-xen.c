...
static struct timer_list sync_xen_wallclock_timer = TIMER_INITIALIZER(sync_xen_wallclock, 0, 0);
static void sync_xen_wallclock(unsigned long dummy)
{
	time_t sec;
	s64 nsec;
...
irqreturn_t timer_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
	...
	/*
	 * Account stolen and blocked ticks.
	 * in RHEL 4 we cannot add a steal time field to /proc/stat
	 * because of compatibility worries, but we can at least make
	 * sure tasks do not get wrongly billed for time the virtual
	 * CPU itself was not running...
	 */
	stolen += blocked;
	if ((stolen > 0) && (delta_cpu > 0)) {
		...
		scheduler_tick(0, (int)stolen);

		/* Make sure the timers are always run. */
		if (unlikely(delta_cpu < NS_PER_TICK))
			run_local_timers();
	}
