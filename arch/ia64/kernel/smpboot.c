...
remove_from_mtinfo(int cpu)
{
	...
	for_each_cpu(i)
		if (mt_info[i].valid &&  mt_info[i].socket_id ==
		    			cpu_data_topology(cpu)->socket_id)
			mt_info[i].valid = 0;
}
...
remove_siblinginfo(int cpu)
{
	...
	if (cpu_data_topology(cpu)->threads_per_core == 1 &&
	    cpu_data_topology(cpu)->cores_per_socket == 1) {
...
set_cpu_sibling_map(int cpu)
{
	...
	for_each_online_cpu(i) {
		if ((cpu_data_topology(cpu)->socket_id ==
				cpu_data_topology(i)->socket_id)) {
			cpu_set(i, cpu_core_map[cpu]);
			cpu_set(cpu, cpu_core_map[i]);
			if (cpu_data_topology(cpu)->core_id ==
					cpu_data_topology(i)->core_id) {
