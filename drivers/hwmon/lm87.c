 ...
 *     as an argument.  So we can't use that to look up pin numbers.
 *     instead we have to create a seperate callback function for every pin.
 *
...
/*
 * Use SENSORS_INSMOD_1 instead of I2C_CLIENT_INSMOD_1.  I2C_CLIENT_INSMOD_1 isn't
 * defined in RHEL4.  This also means we will have to use use i2c_detect instead 
 * of i2c_probe since we are using this macro.
 */
...
static ssize_t set_temp##offset##_low(struct device *dev, const char *buf, \
		size_t count) \
{ \
	set_temp_low(dev, buf, offset-1); \
	return count; \
} \
static ssize_t set_temp##offset##_high(struct device *dev, const char *buf, \
		size_t count) \
{ \
