	...
	while (1) {
		if (!ql_sem_lock(qdev,
			...
			if (i < 65) {
				...
				if(i > 60)
					printk(KERN_ERR PFX "%s: Waited %d sec "
						"for driver lock...\n",
						qdev->ndev->name, i);
...
static int ql_adapter_reset(struct ql3_adapter *qdev)
{
	...
	/*  Issuing the forced reset because a soft reset failed due to
	 *  timing out.  This will reset the hardware without waiting for
	 *  other drivers to respond to the reset interrupt */
	if (max_wait_time == 0) {
		printk(KERN_WARNING PFX
		       "%s: Issueing a forced reset\n",
			qdev->ndev->name);
		...
		do {
			value =
			    ql_read_common_reg(qdev,
					       &port_regs->CommonRegs.
					       ispControlStatus);
			if ((value & (ISP_CONTROL_FSR | ISP_CONTROL_RI)) == 0) {
				break;
			}
			ssleep(1);
		} while ((--max_wait_time));
...
static int ql_adapter_up(struct ql3_adapter *qdev)
{
	...
	if ((err = ql_wait_for_drvr_lock(qdev)) == 0) {
