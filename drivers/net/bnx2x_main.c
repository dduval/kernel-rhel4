...
#define DRV_MODULE_VERSION	"1.45.23-1"
			       ...
			       u16 cqe_idx)
{
	...
	if (frag_size)
		skb_shinfo(skb)->tso_size = min((u32)BCM_PAGE_SIZE,
	...
	while (sw_comp_cons != hw_comp_cons) {
		...
		/* this is an rx packet */
			...
			if ((bp->dev->mtu > ETH_MAX_PACKET_SIZE) &&
				...
				/* aligned copy */
				memcpy(new_skb->data + pad, skb->data + pad, len);
...
#if 0 /* RHEL4 doesn't have ethtool support for LRO */
	/* Set TPA flags */
	if (disable_tpa) {
...
#if 0 /* RHEL4 doesn't have ethtool support for setting flags */
...
#if 0 /* not in RHEL4 */
	if (skb_shinfo(skb)->tso_type & SKB_GSO_TCPV4)
		rc |= XMIT_GSO_V4;
			     ...
			     u32 xmit_type)
{
	...
	if (skb_shinfo(skb)->nr_frags >= (MAX_FETCH_BD - 3)) {
		...
		if (xmit_type & XMIT_GSO) {
			unsigned short lso_mss = skb_shinfo(skb)->tso_size;
			struct tcphdr *tcph = (struct tcphdr *)skb->h.raw;
...
static int bnx2x_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	DP(NETIF_MSG_TX_QUEUED, "SKB: summed %x  protocol %x  protocol(%x,%x)"
	   "  xmit_type %x\n",
	   skb->ip_summed, skb->protocol, ipv6h->nexthdr,
	   iph->protocol, xmit_type);
	...
	*/
	...
	if (xmit_type & XMIT_CSUM) {
		struct tcphdr *tcph = (struct tcphdr *)skb->h.raw;
		hlen = (skb->nh.raw - skb->data + vlan_off) / 2;
		...
		pbd->ip_hlen = skb->nh.iph->ihl << 1;

		hlen += pbd->ip_hlen + tcph->doff * 2;
		...
		if (xmit_type & XMIT_CSUM_TCP) {
			struct tcphdr *tcph = (struct tcphdr *)skb->h.raw;
			pbd->tcp_pseudo_csum = swab16(tcph->check);
	...
	if (xmit_type & XMIT_GSO) {
		...
		pbd->lso_mss = cpu_to_le16(skb_shinfo(skb)->tso_size);
		pbd->tcp_send_seq = swab32(tcph->seq);
		pbd->tcp_flags = pbd_tcp_flags(skb);

		if (xmit_type & XMIT_GSO_V4) {
			pbd->ip_id = swab16(skb->nh.iph->id);
			pbd->tcp_pseudo_csum =
				swab16(~csum_tcpudp_magic(skb->nh.iph->saddr,
							  skb->nh.iph->daddr,
							  0, IPPROTO_TCP, 0));
				    ...
				    struct net_device *dev)
{
	...
	if (pdev->is_enabled) {
		...
		pci_set_master(pdev);
		pci_save_state(pdev, bp->pci_cfg_state);
	}
