...
static inline void ixgbe_irq_enable(struct ixgbe_adapter *adapter)
{
	...
	if (adapter->flags & IXGBE_FLAG_FAN_FAIL_CAPABLE)
		IXGBE_WRITE_REG(&adapter->hw, IXGBE_EIMS,
				IXGBE_EIMS_ENABLE_MASK | IXGBE_EIMS_GPI_SDP1);
	else
		IXGBE_WRITE_REG(&adapter->hw, IXGBE_EIMS,
				IXGBE_EIMS_ENABLE_MASK);
	IXGBE_WRITE_FLUSH(&adapter->hw);
...
static int ixgbe_resume(struct pci_dev *pdev)
{
	...
	pci_restore_state(pdev, adapter->pci_cfg_state);
...
#ifdef CONFIG_PM
	retval = pci_save_state(pdev, adapter->pci_cfg_state);
	if (retval)
		return retval;
#endif
	...
	*hdr_len = 0;
	...
	if (skb_is_tso(skb)) {
		...
		*hdr_len += l4len;
		...
		vlan_macip_lens |= ((skb->nh.raw - skb->data) <<
		...
		*hdr_len +=
		...
		/* MSS L4LEN IDX */
		mss_l4len_idx |=
		    (skb_shinfo(skb)->tso_size << IXGBE_ADVTXD_MSS_SHIFT);
				   ...
				   struct sk_buff *skb, u32 tx_flags)
{
	...
	if (skb->ip_summed == CHECKSUM_PARTIAL ||
		...
		vlan_macip_lens |= ((skb->nh.raw - skb->data) <<
				    IXGBE_ADVTXD_MACLEN_SHIFT);
		if (skb->ip_summed == CHECKSUM_PARTIAL)
			vlan_macip_lens |= (skb->h.raw - skb->nh.raw);
		...
		if (skb->ip_summed == CHECKSUM_PARTIAL) {
			...
			case __constant_htons(ETH_P_IPV6):
				ipv6h = (struct ipv6hdr *)skb->nh.raw;
				 ...
				 const struct pci_device_id *ent)
{
	...
	pci_save_state(pdev, adapter->pci_cfg_state);
