...
#ifdef CONFIG_IGB_LRO
	if (!lro_disable)
		adapter->flags |= IGB_FLAG_LRO_ENABLED;
#endif
	if (pci_using_dac)
			      ...
			      struct sk_buff *skb, u32 tx_flags, u8 *hdr_len)
{
...
#if 0 /* not in RHEL4 */
	else if (skb_shinfo(skb)->tso_type == SKB_GSO_TCPV6) {
	...
	*hdr_len += skb->h.raw - skb->nh.raw;
	...
	/* MSS L4LEN IDX */
	mss_l4len_idx = (skb_shinfo(skb)->tso_size << E1000_ADVTXD_MSS_SHIFT);
				   ...
				   struct igb_ring *tx_ring)
{
	...
	tso = (skb_shinfo(skb)->tso_size) ? igb_tso_adv(adapter, tx_ring, skb, tx_flags,
							&hdr_len) : 0;
...
static irqreturn_t igb_msix_rx(int irq, void *data, struct pt_regs *regs)
{
	...
	if (netif_rx_schedule_prep(rx_ring->adapter->netdev)) {
		rx_ring->total_bytes = 0;
		rx_ring->total_packets = 0;
		__netif_rx_schedule(rx_ring->adapter->netdev);
        }
	return IRQ_HANDLED;
}
 ...
 * @adapter: board private structure
 **/
static int igb_poll(struct net_device *poll_dev, int *budget)
{
	struct igb_adapter *adapter = netdev_priv(poll_dev);
	...
	if (!(adapter->flags & IGB_FLAG_IN_NETPOLL)) {
		/* igb_poll is called per-cpu.  This lock protects tx_ring[i] from
		...
		if (spin_trylock(&adapter->tx_ring[0].tx_clean_lock)) {
			tx_clean_complete &= igb_clean_tx_irq(&adapter->tx_ring[0]);
			spin_unlock(&adapter->tx_ring[0].tx_clean_lock);
		}

		igb_clean_rx_irq_adv(&adapter->rx_ring[0], &work_done, work_to_do);
		*budget -= work_done;
...
static int igb_clean_rx_ring_msix(struct net_device *poll_dev, int *budget)
{
	struct igb_adapter *adapter = netdev_priv(poll_dev);
	...
	*budget -= work_done;
	...
	if ((work_done == 0) || !netif_running(poll_dev)) {
quit_polling:
		netif_rx_complete(poll_dev);
		if (adapter->itr_setting & 3) {
			if (adapter->num_rx_queues == 1)
	...
	while (1) {
		while (i != head) {
			...
			if (skb) {
				...
				/* tso_segs is currently only valid for tcp */
				segs = skb_shinfo(skb)->tso_segs ?: 1;
...
#ifdef CONFIG_IGB_LRO
	if (adapter->flags & IGB_FLAG_LRO_ENABLED &&
...
#ifdef CONFIG_PM
	retval = pci_save_state(pdev, adapter->pm_state);
	if (retval)
		return retval;
#endif
