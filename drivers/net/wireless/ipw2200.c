...
static ssize_t show_event_log(struct device *d, char *buf)
{
...
static ssize_t show_error(struct device *d, char *buf)
{
	struct ipw_priv *priv = dev_get_drvdata(d);
	u32 len = 0, i;
	if (!priv->error)
...
static ssize_t clear_error(struct device *d, const char *buf, size_t count)
{
	struct ipw_priv *priv = dev_get_drvdata(d);
	if (priv->error) {
...
static ssize_t show_cmd_log(struct device *d, char *buf)
{
	struct ipw_priv *priv = dev_get_drvdata(d);
	u32 len = 0, i;
	if (!priv->cmdlog)
...
static ssize_t store_eeprom_delay(struct device *d, const char *buf, size_t count)
{
	struct ipw_priv *p = d->driver_data;
...
static ssize_t store_command_event_reg(struct device *d, const char *buf, size_t count)
{
	u32 reg;
	struct ipw_priv *p = d->driver_data;
...
static ssize_t store_mem_gpio_reg(struct device *d, const char *buf, size_t count)
{
	u32 reg;
	struct ipw_priv *p = d->driver_data;
...
static ssize_t store_indirect_dword(struct device *d, const char *buf, size_t count)
{
	struct ipw_priv *priv = d->driver_data;
...
static ssize_t store_indirect_byte(struct device *d, const char *buf, size_t count)
{
	struct ipw_priv *priv = d->driver_data;
...
static ssize_t store_direct_dword(struct device *d, const char *buf, size_t count)
{
	struct ipw_priv *priv = d->driver_data;
