...
static int num_tx_fifo_len;
static int num_rx_ring_sz;
static int num_rts_frm_len;

module_param_array(tx_fifo_len, uint, num_tx_fifo_len, 0);
module_param_array(rx_ring_sz, uint, num_rx_ring_sz, 0);
module_param_array(rts_frm_len, uint, num_rts_frm_len, 0);
		...
		for (j = 0; j < config->tx_cfg[i].fifo_len - 1; j++) {
			...
			skb =
			    (struct sk_buff *) ((unsigned long) txdp->
						Host_Control);
			if (skb == NULL) {
				memset(txdp, 0, sizeof(struct TxD) *
				       config->max_txds);
				continue;
			}
			...
			dev_kfree_skb(skb);
			memset(txdp, 0, sizeof(struct TxD) * config->max_txds);
			cnt++;
		}
		DBG_PRINT(INTR_DBG,
...
static void tx_intr_handler(struct fifo_info *fifo_data)
{
	...
	while ((!(txdlp->Control_1 & TXD_LIST_OWN_XENA)) &&
		...
		memset(txdlp, 0,
		       (sizeof(struct TxD) * fifo_data->max_txds));
...
static void s2io_reset(struct s2io_nic * sp)
{
	...
	for (i = 0; i < S2IO_MAX_PCI_CONFIG_SPACE_REINIT; i++) {
		...
		pci_restore_state(sp->pdev, sp->pci_cfg_state);
...
s2io_init_nic(struct pci_dev *pdev, const struct pci_device_id *pre)
{
	...
	pci_save_state(sp->pdev, sp->pci_cfg_state);
