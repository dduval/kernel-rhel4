...
void scsi_schedule_eh(struct Scsi_Host *shost)
{
	...
	if (test_and_set_bit(SHOST_RECOVERY, &shost->shost_state) == 0 ||
	    test_and_set_bit(SHOST_CANCEL, &shost->shost_state) == 0) {
...
void scsi_times_out(struct scsi_cmnd *scmd)
{
	...
	if (scmd->device->host->hostt->eh_timed_out)
		...
		case EH_RESET_TIMER:
			 ...
			 * with allowed == 0 */
			if (scmd->device->sdev_state != SDEV_BLOCK &&
...
static int scsi_request_sense(struct scsi_cmnd *scmd)
{
	...
	scsi_result = kmalloc(252, GFP_ATOMIC | 
			      ((scmd->device->host->hostt->unchecked_isa_dma) ? __GFP_DMA : 0));
...
static int scsi_eh_try_stu(struct scsi_cmnd *scmd)
{
	...
	for (i = 0; rtn == NEEDS_RETRY && i < 2; i++)
		rtn = scsi_send_eh_cmnd(scmd, scmd->device->timeout);
