...
static int asd_init_scbs(struct asd_ha_struct *asd_ha)
{
	...
	asd_ha->seq.tc_index_array = kmalloc(asd_ha->seq.tc_index_bitmap_bits*
					     sizeof(void *), GFP_KERNEL);
	if (!asd_ha->seq.tc_index_array)
		return -ENOMEM;
	memset(asd_ha->seq.tc_index_array, 0, asd_ha->seq.tc_index_bitmap_bits*
	                                             sizeof(void *));

	bitmap_bytes = (asd_ha->seq.tc_index_bitmap_bits+7)/8;
	bitmap_bytes = BITS_TO_LONGS(bitmap_bytes*8)*sizeof(unsigned long);
	asd_ha->seq.tc_index_bitmap = kmalloc(bitmap_bytes, GFP_KERNEL);
...
static int asd_init_ctxmem(struct asd_ha_struct *asd_ha)
{
	...
	bitmap_bytes = BITS_TO_LONGS(bitmap_bytes*8)*sizeof(unsigned long);
	asd_ha->hw_prof.ddb_bitmap = kmalloc(bitmap_bytes, GFP_KERNEL);
