	...
	struct st_ccb *ccb, struct status_msg *resp)
{
	...
	sg = ccb->cmd->request_buffer;
	/* determine inquiry command status by DeviceTypeQualifier */
	if (ccb->cmd->cmnd[0] == INQUIRY &&
		resp->scsi_status == SAM_STAT_GOOD) {
		ST_INQ *inq_data;
		if (ccb->cmd->use_sg)
			inq_data = kmap_atomic(sg->page, KM_IRQ0) + sg->offset;
		else
			inq_data = ccb->cmd->request_buffer;
...
static int stex_proc_info(struct Scsi_Host *host, char *buffer, char **start,
...
static int stex_sanity_check(struct scsi_device *sdev)
{
	struct st_hba *hba;

	if (!sdev->host || !sdev->host->hostdata)
		return -ENXIO;

	hba = (struct st_hba *)sdev->host->hostdata;
...
static void stex_poll(struct scsi_device *sdev)
{
	struct st_hba *hba;
	void __iomem *base;
	u32 data;
...
static int stex_dump_shutdown(struct scsi_device *sdev)
{
	int count = 0;
	u16 tag;
	struct st_hba *hba;
	...
	hba = (struct st_hba *)sdev->host->hostdata;
	spin_lock(hba->host->host_lock);
	if((tag = stex_alloc_tag((unsigned long *)&hba->tag)) == TAG_BITMAP_LENGTH) {
		spin_unlock(hba->host->host_lock);
		printk("shasta: unable to alloc tag\n");
		return -EIO;
	}
	hba->ccb[tag].req_type = PASSTHRU_REQ_NO_WAKEUP;
...
static struct scsi_host_template driver_template = {
	...
	.dump_sanity_check		= stex_sanity_check,
	.dump_shutdown			= stex_dump_shutdown,
	.dump_poll			= stex_poll,
};
...
static void stex_shutdown(struct device *device)
{
	struct st_hba *hba = pci_get_drvdata(to_pci_dev(device));
