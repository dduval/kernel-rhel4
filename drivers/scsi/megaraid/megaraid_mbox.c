...
static void megaraid_diskdump_sleep(unsigned int timeout);


extern int mraid_mm_woken;
...
static struct scsi_host_template megaraid_template_g = {
	...
	.dump_sanity_check		= megaraid_sanity_check,
	.dump_quiesce			= megaraid_quiesce,
	.dump_poll			= megaraid_poll,
};
...
extern struct scsi_device *mraid_mm_diskdump_poll_device;
extern void (*mraid_mm_poll_func)(struct scsi_device *device);

static int
megaraid_sanity_check(struct scsi_device *device)
{
        adapter_t       *adapter;

        adapter = (adapter_t *)SCSIHOST2ADAP(device->host);
        if ( adapter == NULL ) {
                return -ENXIO;
        }
...
#endif
	mraid_mm_poll_func = megaraid_poll;
	mraid_mm_diskdump_poll_device = device;
        return 0;
}


static int
megaraid_quiesce(struct scsi_device *device)
{
	adapter_t	*adapter;

	adapter = (adapter_t *)SCSIHOST2ADAP(device->host);
	/*
	 * In case of crash dump, the tasklet list has been already abandoned
	 * so the tasklet_struct must be reinitialized.
	 * If it is not done, the tasklet is not scheduled because of garbage.
	 */
	tasklet_init(&adapter->dpc_h, megaraid_mbox_dpc,
		(unsigned long)adapter);
...
static void
megaraid_poll(struct scsi_device *device)
{
        adapter_t       *adapter;
        u32             dword = 0;
	mraid_device_t	*raid_dev;
	...
	raid_dev = ADAP2RAIDDEV(adapter);
        ...
        megaraid_isr(0, adapter, NULL);
}


static void
megaraid_diskdump_sleep(unsigned int timeout)
{
        unsigned long expire;

	if (!mraid_mm_diskdump_poll_device) {
		return;
	}
		...
		megaraid_poll(mraid_mm_diskdump_poll_device);
                udelay(100);
                diskdump_update();
                if (mraid_mm_woken)
                        return;
