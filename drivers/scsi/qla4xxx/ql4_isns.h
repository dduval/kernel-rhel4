...
typedef struct _ISNS_DISCOVERED_TARGET_PORTAL {
	uint8_t       IPAddr[16];		  /* x00-x0F */
	uint16_t      PortNumber;           /* x10-x13 */
} ISNS_DISCOVERED_TARGET_PORTAL, *PISNS_DISCOVERED_TARGET_PORTAL;

typedef struct _ISNS_DISCOVERED_TARGET {
	uint32_t      NumPortals;		  /*  x00-x03 */
#define ISNS_MAX_PORTALS 4
	ISNS_DISCOVERED_TARGET_PORTAL Portal[ISNS_MAX_PORTALS];	/* x04-x53 */
	uint32_t      DDID;		  /*  x54-x57 */
	uint8_t       NameString[256];	  /*  x58-x157 */
	uint8_t       Alias[32];		  /* x158-x177 */
