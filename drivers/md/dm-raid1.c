...
static void complete_resync_work(struct region *reg, int success)
{
	...
	BUG_ON(!(reg->state == RH_RECOVERING || \
		 (!success && reg->state == RH_NOSYNC)));
	if (!success)
		DMERR("recovery failed on region %lld", reg->key);

	rh->log->type->complete_resync_work(rh->log, reg->key, success);
			      ...
			      void *context)
{
	...
	if (read_err) {
		/* A read error means the default mirror failed. */
 ...
 * @ms: the mirror set
 *
 * This function is used for read balancing and should only
 * be called if the region we are reading is in-sync.
 *
 * Returns: chosen mirror, or NULL on failure
 */
static struct mirror *choose_mirror(struct mirror_set *ms)
{
	struct mirror *ret = ms->read_mirror;
...
static void fail_mirror(struct mirror *m)
{
	...
	if (!ms->in_sync) {
		/*
		 * Cannot switch default mirror.
		 * Better to fail than read potentially bad data.
		 */
		DMERR("Primary mirror device has failed while mirror is out of sync.");
	...
	if (unlikely(new == ms->mirror + ms->nr_mirrors))
		DMWARN("All mirrors of %s have failed.", ms->mirror->dev->name);
}
...
static void read_callback(unsigned long error, void *context)
{
	...
	if (unlikely(error)) {
		...
		if (likely(default_ok(m)) || mirror_available(m->ms, bio)) {
			DMWARN("Read failure on mirror: Trying different device.");
			queue_bio(m->ms, bio, bio_rw(bio));
		} else {
			DMERR("Read failure on mirror: Failing I/O.");
			      ...
			      struct bio *bio, unsigned int done, int error)
{
	...
	log->type->complete_resync_work(log, region, 0);
	ms->in_sync = 0;
		      ...
		      union map_info *map_context)
{
	...
	if (likely(r)) {
		...
		if (likely(m)) {
			...
			map_bio(m, bio);
			return DM_MAPIO_REMAPPED; /* queue request */
		} else
			return -EIO;
	} else {
			 ...
			 int error, union map_info *map_context)
{
	...
	if (unlikely(error)) {
		...
		if (default_ok(m) || mirror_available(ms, bio)) {
			...
			map_context->ptr = NULL;
			queue_bio(ms, bio, rw);
			/* We want another shot on the bio. */
...
static int __init dm_mirror_init(void)
{
	...
	if (r < 0) {
		...
		DMWARN("If this happens, you must:");
		DMWARN("  1) Switch to core logging");
		DMWARN("  2) Allow to resync");
		DMWARN("  3) Switch back to disk logging");
	}
...
MODULE_PARM_DESC(dm_mirror_error_on_log_failure, "Set whether writes will fail on log device failure");
