 ...
 */
static ssize_t
dasd_erplog_show(struct device *dev, char *buf)
{
	struct dasd_devmap *devmap;
	int erplog;
...
static ssize_t
dasd_erplog_store(struct device *dev, const char *buf, size_t count)
{
	struct dasd_devmap *devmap;
	int val;
	char *endp;
	...
	if (devmap->device) {
		if (val)
			set_bit(DASD_FLAG_ERPLOG, &devmap->device->flags);
		else
			clear_bit(DASD_FLAG_ERPLOG, &devmap->device->flags);
	}
	spin_unlock(&dasd_devmap_lock);
	return count;
}
...
static ssize_t
dasd_alias_show(struct device *dev, char *buf)
{
	struct dasd_devmap *devmap;
	int alias;
...
static ssize_t
dasd_vendor_show(struct device *dev, char *buf)
{
	struct dasd_devmap *devmap;
	char *vendor;
...
static ssize_t
dasd_uid_show(struct device *dev, char *buf)
{
	struct dasd_devmap *devmap;
	char uid[UID_STRLEN];
