...
#ifdef __powerpc64__
	/*
	 * <benh>  mdelay has a funny bug with micropartitionning
	 * <arjan> worst case you implement mdelay() as a for loop of udelay's
	 * <benh>  yah, and that's why it's buggy :)
	 * <benh>  current ppc64 mdelay is a for loop of udelay(1000)
	 * <arjan> how is such a for loop worse than udelay(100 * 1000) ?
	 * <benh>  because it breaks with micropartitionning :)
	 * <benh>  your partition gets a 1 ms timeslice every 10ms
	 * <benh>  the udelay is locked on the timebase, so it actually ends up getting ... 10ms instead of 1 :)
	 * <benh>  thus you end up waiting 10 times too much
	 * <benh>  because udelay does tstart = timebase; then loop until timebase is tstart + 1ms
	 * <benh>  with micropartitionning, you'll only "end" on the next slice
	 * <benh>  which is 10ms later
	 * <benh>  if we did udelay(N*1000) it would work
	 * <benh>  but because we do a loop of udelay(1000); it's fucked :)
	 * <arjan> doesn't sound too impossible to change it
	 * <benh>  yes, we need to turn it into udelay(N*1000);
	 * <benh>  but anyway, using mdelay is generally broken, so ...
	 */
	udelay(100 * 1000);
#else
	mdelay(100);
