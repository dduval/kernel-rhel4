...
#define DRIVER_NAME "HP CISS Driver (v 2.6.20.RH2)"
#define DRIVER_VERSION CCISS_DRIVER_VERSION(2,6,20)

/* Embedded module documentation macros - see modules.h */
MODULE_AUTHOR("Hewlett-Packard Company");
MODULE_DESCRIPTION("Driver for HP Controller SA5xxx SA6xxx version 2.6.20.RH2");
MODULE_VERSION("2.6.20.RH2");
...
static struct board_type products[] = {
	...
	{ 0x323D103C, "Smart Array 700m", &SA5_access},
...
static int deregister_disk(ctlr_info_t *h, int drv_index,
			   int clear_all);
static void cciss_del_disk_sysfs_and_gendisk(struct gendisk *disk);
...
static ssize_t cciss_show_vendor(struct device *dev, char *buf)
{
	struct drv_dynamic *d;
	drive_info_struct *drv;
	ctlr_info_t *h;
...
static ssize_t cciss_show_model(struct device *dev, char *buf)
{
	struct drv_dynamic *d;
	drive_info_struct *drv;
	ctlr_info_t *h;
			...
			int withirq, drive_info_struct *drv)
{
	...
	if (withirq)
		...
		return_code = sendcmd(CISS_INQUIRY, ctlr, inq_buff,
			sizeof(*inq_buff), 1, logvol , 0, NULL, TYPE_CMD, 0, 0);
	if (return_code == IO_OK) {
...
static void cciss_update_drive_info(int ctlr, int drv_index, int first_time)
{
	...
	if (h->drv[drv_index].raid_level != -1 &&
		((memcmp(drvinfo->uid, h->drv[drv_index].uid, 16) == 0) &&
	...
	memcpy(h->drv[drv_index].uid, drvinfo->uid, sizeof(drvinfo->uid));
	 ...
	 * then we want to update the logical drive's information.
	 */
 ...
 * "is_controller_node" is used to know if this is a real logical drive, or just
...
static void cciss_del_disk_sysfs_and_gendisk(struct gendisk *disk)
{
	...
	d = container_of(drv->dev_info, struct drv_dynamic, dev);
	for (i = 0; cciss_sysfs_attrs[i] != NULL;i++)
		sysfs_remove_file(&d->dev.kobj, cciss_sysfs_attrs[i]);
	d->disk = NULL;
	del_gendisk(disk);
	device_unregister(drv->dev_info);
	drv->dev_info = NULL;
	...
	*/
	if (h->gendisk[0] != disk){
		if (disk->flags & GENHD_FL_UP)
			cciss_del_disk_sysfs_and_gendisk(disk);		
		if (disk->queue) {
