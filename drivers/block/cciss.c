...
#define DRIVER_NAME "HP CISS Driver (v 2.6.20.RH1)"
#define DRIVER_VERSION CCISS_DRIVER_VERSION(2,6,20)

/* Embedded module documentation macros - see modules.h */
MODULE_AUTHOR("Hewlett-Packard Company");
MODULE_DESCRIPTION("Driver for HP Controller SA5xxx SA6xxx version 2.6.20.RH1");
MODULE_VERSION("2.6.20");
...
static struct board_type products[] = {
	...
	{ 0x3240103C, "Smart Array 344", &SA5_access},
	{ 0x3241103C, "Smart Array 544", &SA5_access},
	{ 0x3242103C, "Smart Array 380", &SA5_access},
	{ 0x3243103C, "Smart Array 580", &SA5_access},
	{ 0x3244103C, "Smart Array 380i", &SA5_access},
	{ 0x3245103C, "Smart Array 580i", &SA5_access},
	{ 0x3246103C, "Smart Array 308", &SA5_access},
	{ 0x3247103C, "Smart Array 508", &SA5_access},
	{ 0x3248103C, "Smart Array 388", &SA5_access},
	{ 0x3249103C, "Smart Array 588", &SA5_access},
...
static int sendcmd( __u8 cmd, int ctlr, void *buff, size_t size,
	...
	unsigned char *scsi3addr, int cmd_type, int block_nr, int diskdump);
...
static ssize_t cciss_show_raid_level(struct device *dev, char *buf)
{
...
static ssize_t cciss_show_disk_size(struct device *dev, char *buf)
{
	struct drv_dynamic *d;
	drive_info_struct *drv;
	ctlr_info_t *h;
...
static ssize_t cciss_show_usage_count(struct device *dev, char *buf)
{
	struct drv_dynamic *d;
	drive_info_struct *drv;
	int count;
...
static ssize_t cciss_show_dump(struct device *dev, char *buf)
{
	struct drv_dynamic *d;
	struct gendisk *disk;
	ssize_t res;
	...
	disk = d->disk;
	kobject_get(&disk->kobj);
	spin_unlock(&sysfs_lock);
	res = diskdump_sysfs_show_disk(disk, buf);
	kobject_put(&disk->kobj);
	return res;
}

static ssize_t cciss_store_dump(struct device *dev, const char *buf, size_t count)
{
	struct drv_dynamic *d;
	struct gendisk *disk;
	ssize_t res;
	...
	kobject_get(&disk->kobj);
	spin_unlock(&sysfs_lock);
	res = diskdump_sysfs_store_disk(disk, dev, buf, count);
	kobject_put(&disk->kobj);
	return res;
}

DEVICE_ATTR(dump, S_IRUGO | S_IWUSR, cciss_show_dump, cciss_store_dump);
#endif
...
#ifdef CONFIG_CCISS_DUMP
	d->sanity_check = cciss_dump_sanity_check;
	d->rw_block = cciss_dump_rw_block;
	d->quiesce = cciss_dump_quiesce;
	d->shutdown = cciss_dump_shutdown;
	d->probe = cciss_probe;
	d->block_add_device = cciss_add_device;
#endif
...
static CommandList_struct * cmd_alloc(ctlr_info_t *h, int get_from_pool)
{
	...
	{
	     	do {
                	i = find_first_zero_bit(h->cmd_pool_bits, h->max_nr_cmds);
                        if (i == h->max_nr_cmds)
                                return NULL;
...
cleanup1:
	...
	case SCSI_IOCTL_SEND_COMMAND:
		/* scsi_cmd_ioctl is broken for this on RHEL4. */
...
geo_inq:
	...
	if (drv_index){
		...
		sprintf(disk->devfs_name, "cciss/host%d/target%d", ctlr, drv_index);
		disk->major = h->major;
	...
	*/
	if (h->gendisk[0] != disk){
		...
		if (disk->queue) {
			...
			/* Set drv->queue to NULL so that we do not try to
			 * call blk_start_queue on this queue in the
			 * interrupt handler
			 */
			drv->queue = NULL;
		}
		/* If clear_all is set then we are deleting the logical
		 * drive, not just refreshing its info.  For drives other
		 * than disk 0 we will call put_disk.  We do not do this
		 * for disk 0 as we need it to be able to configure the
		 * controller.
		 */
		if (clear_all) {
			/* This isn't pretty, but we need to find the disk
			 * in our array and NULL out the pointer.  this is
			 * so that we will call alloc_disk if this index
			 * is used again later.
			 */
			for (i=0; i < CISS_MAX_LUN; i++){
				if(h->gendisk[i] == disk){
	...
	sector_t block_nr)
{
	...
	if (cmd_type == TYPE_CMD) {
		...
		case CCISS_READ_16:
			...
			c->Request.Type.Direction = XFER_READ;
			c->Request.Timeout = 0;
			c->Request.CDB[0] = CCISS_READ_16;
			c->Request.CDB[1]= 0;
			c->Request.CDB[2]= (block_nr >> 56) & 0xff;    //MSB
			c->Request.CDB[3]= (block_nr >> 48) & 0xff;
			c->Request.CDB[4]= (block_nr >> 40) & 0xff;
			c->Request.CDB[5]= (block_nr >> 32) & 0xff;
			c->Request.CDB[6]= (block_nr >> 24) & 0xff;
			c->Request.CDB[7]= (block_nr >> 16) & 0xff;
			c->Request.CDB[8]= (block_nr >>  8) & 0xff;
			c->Request.CDB[9]= block_nr & 0xff;
			nr_blocks = size/hba[ctlr]->drv[log_unit].block_size;
			c->Request.CDB[10]= (nr_blocks >>  24) & 0xff;
			c->Request.CDB[11]= (nr_blocks >>  16) & 0xff;
			c->Request.CDB[12]= (nr_blocks >>  8) & 0xff;
			c->Request.CDB[13]= nr_blocks & 0xff;
		...
		case CCISS_WRITE_10:
			...
			c->Request.Type.Direction = XFER_WRITE;
			c->Request.Timeout = 0;
			c->Request.CDB[0] = CCISS_WRITE_10;
			c->Request.CDB[2] = (block_nr >> 24) & 0xff;
			c->Request.CDB[3] = (block_nr >> 16) & 0xff;
			c->Request.CDB[4] = (block_nr >> 8) & 0xff;
			c->Request.CDB[5] = block_nr & 0xff;
		...
		case CCISS_WRITE_16:
			...
			c->Request.Type.Direction = XFER_WRITE;
			c->Request.Timeout = 0;
			c->Request.CDB[0] = CCISS_WRITE_16;
			c->Request.CDB[1]= 0;
			c->Request.CDB[2]= (block_nr >> 56) & 0xff;    //MSB
			c->Request.CDB[3]= (block_nr >> 48) & 0xff;
			c->Request.CDB[4]= (block_nr >> 40) & 0xff;
	...
	int cmd_type)
{
	...
	return_status = fill_cmd(c, cmd, ctlr, buff, size, use_unit_num,
			log_unit, page_code, NULL, cmd_type, 0);
			...
			drive_info_struct *drv)
{
	...
	if (withirq)
		...
		return_code = sendcmd(CISS_INQUIRY, ctlr, inq_buff,
			sizeof(*inq_buff), 1, logvol ,0xC1, NULL, TYPE_CMD, 0, 0);
	if (return_code == IO_OK) {
				...
				unsigned int *block_size)
{
	...
	if (withirq)
		...
		return_code = sendcmd(CCISS_READ_CAPACITY,
			ctlr, buf, sizeof(*buf), 1, logvol, 0, NULL, TYPE_CMD, 0, 0);
	if (return_code == IO_OK) {
				   ...
				   1: this is a diskdump command */
{
	...
	status = fill_cmd(c, cmd, ctlr, buff, size, use_unit_num,
		log_unit, page_code, scsi3addr, cmd_type, block_nr);
	if (status != IO_OK) {
		if (diskdump != 1)
			cmd_free(info_p, c, 1);
		return status;
	...
	switch (cmd->err_info->CommandStatus) {
		...
		case CMD_INVALID:
			...
			rq->errors = make_status_bytes(SAM_STAT_GOOD,
				...
				blk_pc_request(rq) ? DID_PASSTHROUGH :
				DID_ERROR);
			break;
		case CMD_PROTOCOL_ERR:
			printk(KERN_WARNING "cciss: cmd %p has "
...
after_error_processing:
	...
	complete_buffers(cmd->rq->bio, rq->errors);
...
#				endif
	 ...
	 */
 	if ((find_first_zero_bit(h->cmd_pool_bits, h->max_nr_cmds)) == h->max_nr_cmds)
 		goto cleanup;
...
{
#if defined (CONFIG_PCI_MSI) && !defined (CONFIG_IA64)
        int err;
...
default_int_mode:
#endif /* CONFIG_PCI_MSI  && !CONFIG_IA64 */
...
static void cciss_getgeometry(int cntl_num)
{
	...
	return_code = sendcmd(CISS_INQUIRY, cntl_num, inq_buff, 
		sizeof(InquiryData_struct), 0, 0 ,0, NULL, TYPE_CMD, 0, 0);
	if (return_code == IO_OK)
	{
	...
	return_code = sendcmd(CISS_REPORT_LOG, cntl_num, ld_buff, 
			sizeof(ReportLunData_struct), 0, 0, 0, NULL, TYPE_CMD, 0, 0);
	...
	for(i=0; i < CISS_MAX_LUN; i++)
	{
		if (i < hba[cntl_num]->num_luns){
	  	...
	  	printk(KERN_DEBUG "LUN[%d]:  %x %x %x %x = %x\n", i, 
			...
			cciss_read_capacity(cntl_num, i, 0,
					&total_size, &block_size);
			/*
			 * if read_capacity returns all F's this volume
			 * is >2TB in size so we switch to 16-byte CDB's
			 * for all read/write ops
			 */
			if (total_size == 0xFFFFFFFFULL) {
				cciss_read_capacity_16(cntl_num, i, 0,
					&total_size, &block_size);
	...
	const struct pci_device_id *ent)
{
	...
	hba[i]->cmd_pool_bits = kmalloc(((hba[i]->max_nr_cmds+BITS_PER_LONG-1)/BITS_PER_LONG)*sizeof(unsigned long), GFP_KERNEL);
	hba[i]->cmd_pool = (CommandList_struct *)pci_alloc_consistent(
		hba[i]->pdev, hba[i]->max_nr_cmds * sizeof(CommandList_struct), 
		&(hba[i]->cmd_pool_dhandle));
	hba[i]->errinfo_pool = (ErrorInfo_struct *)pci_alloc_consistent(
		hba[i]->pdev, hba[i]->max_nr_cmds * sizeof( ErrorInfo_struct), 
...
clean4:
	...
	if(hba[i]->cmd_pool)
		pci_free_consistent(hba[i]->pdev,
			hba[i]->max_nr_cmds * sizeof(CommandList_struct),
			hba[i]->cmd_pool, hba[i]->cmd_pool_dhandle);
	if(hba[i]->errinfo_pool)
		pci_free_consistent(hba[i]->pdev,
			hba[i]->max_nr_cmds * sizeof( ErrorInfo_struct),
			hba[i]->errinfo_pool,
			hba[i]->errinfo_pool_dhandle);
...
static void __devexit cciss_remove_one (struct pci_dev *pdev)
{
	...
	* To write all data in the battery backed cache to disks */
	...
	pci_free_consistent(hba[i]->pdev, hba[i]->max_nr_cmds * sizeof(CommandList_struct),
			    hba[i]->cmd_pool, hba[i]->cmd_pool_dhandle);
	pci_free_consistent(hba[i]->pdev, hba[i]->max_nr_cmds * sizeof( ErrorInfo_struct),
