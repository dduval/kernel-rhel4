				      ...
				      const struct pci_device_id *id)
{
	...
	err = request_irq(vp_dev->pci_dev->irq, vp_interrupt, SA_SHIRQ,
...
#ifdef CONFIG_PM
static int virtio_pci_suspend(struct pci_dev *pci_dev, u32 state)
{
	struct virtio_pci_device *vp_dev = pci_get_drvdata(pci_dev);

	pci_save_state(pci_dev, vp_dev->pci_state);
...
static int virtio_pci_resume(struct pci_dev *pci_dev)
{
	...
	pci_restore_state(pci_dev, vp_dev->pci_state);
