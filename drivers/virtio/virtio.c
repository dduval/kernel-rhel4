...
static int virtio_hotplug(struct device *_dv, char **envp, int num_envp,
			  char *buffer, int buffer_size)
{
	...
	envp[i++] = scratch;
	length += scnprintf (scratch, buffer_size - length, "VENDOR_ID=0x%06x",
			    dev->id.vendor);
	...
	envp[i++] = scratch;
	length += scnprintf (scratch, buffer_size - length, "DEVICE_ID=0x%06x",
			    dev->id.device);
	if ((buffer_size - length <= 0) || (i >= num_envp))
		return -ENOMEM;
	++length;
