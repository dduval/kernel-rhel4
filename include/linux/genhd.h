...
{
	switch( offset ) {
	case disk_stats_index(reads):
		part->reads += addnd;
		break;
	case disk_stats_index(read_sectors):
		part->read_sectors += addnd;
		break;
	case disk_stats_index(writes):
		part->writes += addnd;
		break;
	case disk_stats_index(write_sectors):
		part->write_sectors += addnd;
		break;
	}
}
